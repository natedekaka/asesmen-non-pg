...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($judul === '' || $durasi < 1 || !in_array($status, ['draft', 'aktif', 'selesai'])) {
        ...
        $stmt = $conn->prepare("UPDATE ujian SET judul=?, mata_pelajaran=?, deskripsi=?, tanggal_mulai=?, tanggal_selesai=?, durasi_menit=?, status=?, acak_soal=?, tampilkan_hasil=? WHERE id=?");
        $stmt->bind_param('ssssssiiii', $judul, $mapel, $deskripsi, $mulai, $selesai, $durasi, $status, $acak, $tampil, $id);
        $stmt->execute();
        flash_set('sukses', 'Ujian berhasil diperbarui.');
        redirect('index.php');
    }
}
...
<div class="flex items-center justify-between">
  <h1 class="text-2xl font-bold text-slate-800">Edit Ujian</h1>
